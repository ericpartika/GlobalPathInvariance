...
class State{
  ...
  } CurrentState, PreviousState, UpdatedState;  
...
void Control (double T)
{
  ...
  x4 = CurrentState.x4;
   ...
   double Vx = ( CurrentState.x1 - PreviousState.x1 ) /dt;
   double Vy = ( CurrentState.x2 - PreviousState.x2 ) /dt;
   PreviousState = CurrentState;
   
   double consX5 = sqrt(Vx*Vx + Vy*Vy) - v;
   //CurrentState.set_values( CurrentState.x1, CurrentState.x2, CurrentState.x3, CurrentState.x4, consX5, CurrentState.x6);
   double consX6 = ( consX5 - CurrentState.x5 )/dt;
   //CurrentState.set_values(CurrentState.x1, CurrentState.x2, CurrentState.x3, CurrentState.x4, consX5, consX6);
...
tan(x4)*2.0+vfixed*(x1*x1*x1)*x5*sin(x3)*tan(x4)*2.0-vfixed*(x1*x1)*x2*x5*cos(x3)*tan(x4)*2.0+vfixed*x1*(x2*x2)*x5*sin(x3)*tan(x4)*2.0+L*vfixed*x1*x2*x5*sin(x3*2.0)*8.0);
   ...
   // Control inputs
    ...
    //PreviousState.set_values( CurrentState.x1, CurrentState.x2, CurrentState.x3, CurrentState.x4, CurrentState.x5, CurrentState.x6 );
    //CurrentState.set_values( x1,x2,x3,x4, CurrentState.x5, CurrentState.x6);
    
    UpdatedState.set_values( x1,x2,x3,x4,x5,x6 );
    
    CurrentState = UpdatedState;
    
    ROS_INFO("Est States: %f, %f, %f, %f, %f, %f %f", x1, x2, x3, x4, x5, x6, eta2);
...
int main(int argc, char **argv)
{
  ...
  double T = 0;
    ...
    PreviousState.set_values(x1,x2,x3,x4,x5,x6);
    CurrentState.set_values(x1,x2 + 0.002,x3,x4,x5,x6);
